...
export default function KDramaPage() {
  ...
  async function fetchFavorites(currentOffset: number, append = false, status?: string) {
    ...
    const res = await fetch(`/api/favorites?type=kdrama&status=${apiStatus}&limit=25&offset=${currentOffset}`);
    if (!res.ok) return;
  ...
  if (search.trim()) { const q = search.toLowerCase(); activeItems = activeItems.filter(f => f.title.toLowerCase().includes(q)); }
    ...
    <div className="min-h-screen overflow-y-auto relative z-10">
      <div className="max-w-5xl mx-auto px-4 pt-20 pb-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-2xl font-semibold text-[#2d2640]">K-Drama</h1>
          </div>
        ...
        {showAddForm && <div className="mb-8"><AddFavoriteForm type="kdrama" onAdd={handleAdd} onCancel={() => setShowAddForm(false)} /></div>}
        ...
        <div className="mb-4">
          <GlassTabs tabs={SECTION_ORDER} active={activeTab} onChange={(tab) => { setActiveTab(tab); setOffset(0); fetchFavorites(0, false, statusGroupToApi[tab]); }} layoutId="kdrama-tab" />
        </div>
        ...
        {loading ? (
          ...
          <div>
            {activeItems.length === 0 ? (
              <p className="text-center text-[#7c7291] py-16 text-sm">{activeTab === 'Todo' ? 'Nothing in your list. Add some K-dramas!' : `No ${activeTab.toLowerCase()} dramas.`}</p>
            ) : (
